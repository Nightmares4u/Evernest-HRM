...
The EN HRM + CRM app operates natively within the `EN HRM` monorepo on the `crm-dev` branch (currently experimenting on `ui-revamp-experiment`). The system successfully manages both HR functions and a highly customized CRM flow.
...
- **Migrations:** No `supabase/migrations/` files.
