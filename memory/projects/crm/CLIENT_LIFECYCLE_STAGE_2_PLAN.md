...
> **Implementation note:** Stage 2A-2E has landed and is currently staged
> for final audit on `review/main-plus-ui`. Treat
> §8 schema sketches as historical unless confirmed against migrations
> `0015`, `0017`, `0018`, `0019`, `0020`, `0021`, `0022`, and `0023`.
...
### Phase 2F-1 hardening follow-up — Atomic conversion + PKR-only writes (2026-06-03)
- Migration `0023_crm_convert_lead_to_client_rpc.sql`.
- Applied manually in Supabase before app deployment.
- Adds `public.crm_convert_lead_to_client`.
- `convertLeadToClient` now calls the RPC instead of creating the client,
  payment, and activity through sequential Supabase JS writes.
- The RPC validates converted lead status and assigned-counselor-or-super_admin
  authority, locks the lead row, creates the client shell, creates the
  initial advance payment, and writes the `client_created` activity in one
  database transaction.
- CRM financial writes are PKR-only. Editable currency inputs are removed or
  hidden in conversion, payment, refund, and application financial forms; UI
  displays fixed "PKR only"; server actions force PKR.
- `0023` adds `NOT VALID` constraints enforcing PKR for new/future writes:
  `crm_clients.currency`, `crm_client_payments.currency`,
  `crm_client_refunds.currency`, and
  `crm_client_applications.offer_amount_currency`.
- `NOT VALID` does not validate historical rows at apply time, but new
  inserts and future updates must satisfy the PKR checks.
- Multi-currency remains explicitly deferred.

**Exit criteria:** A converted lead with agreement and advance payment creates
exactly one client, one initial advance payment, and one client-created activity
through `crm_convert_lead_to_client`, with all financial currency values fixed
to PKR.
...
- `app/(dashboard)/crm/clients/actions.ts` —
  historical note only. `recordClientPayment` was moved to RPC by `0022`;
  `convertLeadToClient` was moved to `crm_convert_lead_to_client` by `0023`.
...
- Phase 2A: `convertLeadToClient` and `recordClientPayment` are no longer
  open RPC backlog items. `recordClientPayment` is covered by `0022`;
  `convertLeadToClient` is covered by `0023`.
...
- Phase 2D: `setMilestoneStatus`, `updateClientStatusWithActivity`,
  `ensureClientMilestonesSeeded` are covered by `0022`.
