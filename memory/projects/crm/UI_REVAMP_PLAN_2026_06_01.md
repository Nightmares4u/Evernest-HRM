...
> **Integration status 2026-06-03:** `ui-revamp-experiment` has been merged with `--no-ff --no-commit` into `review/main-plus-ui` for final audit. The integration branch is based on `origin/main`, includes CRM work from `crm-dev`, and is staged only. No integration commit, push, or merge into `main` has occurred.
...
2026-06-03 audit note: the staged integration includes broad UI polish, grouped app shell/sidebar, logo/icon work, shared components, lifecycle tabs, and admin/CRM page updates. It also includes non-UI blocker fixes required by the merge audit: PKR-only hardening, assistant production hardening, and migration `0023` for atomic lead-to-client conversion. Those hardening changes are documented in `CURRENT_STATE.md` and `CLIENT_LIFECYCLE_STAGE_2_PLAN.md`.
...
- **Implemented audit note:** The staged Sidebar maps string icon keys to Lucide components inside the client component. The server layout passes icon keys, avoiding the server-to-client component/function boundary issue.
...
- **Implemented audit note:** The staged integration includes an assistant floating access component, but the assistant remains route-backed at `/crm/assistant`. Final browser audit should verify whether the UX behaves as desired before accepting this as complete.
...
- **No Schema Changes for UI-only work:** Do not add or alter database columns for purely visual revamp tasks.
- **No UI-driven Migrations:** The staged integration contains migration `0023`, but it is a CRM transactional/PKR hardening migration, not a UI revamp migration.
...
- [ ] CRM conversion, payment, refund, and application financial forms display fixed "PKR only" and have no editable currency field.
- [ ] Lead-to-client conversion works after manual `0023` apply.
- [ ] Payroll preview/export exemption fix from `origin/main` is still visible and functional after the UI merge.
